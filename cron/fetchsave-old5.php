...
/* constants */
$comment_text = "本条信息已成功聚合至99haizi.sinaapp.com，请点击查看：http://99haizi.sinaapp.com/ （可以批量看图；分省份查看。）";
...
foreach ($weibos as $weibo) {
	...
	if ($fields["photo_url"] != "") { //only save weibo with photo
		...
		if ($succ) {
			$new_comment = $comment_text." #$weibo_id";
			//post comment
			$furl->setMethod('POST');
			...
			$url_comment .= "&id=$weibo_id&comment=".urlencode($new_comment);
